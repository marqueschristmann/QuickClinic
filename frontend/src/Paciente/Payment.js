...
const Payment = (props) => {
  ...
  function createEvent(id, dateTime, clinicaEmail) {
    var virtualEvent = {
      id: id,
      summary: "Agendamento",
      location: "Virtual",
      description: "Clinica-Paciente agendamento",
      start: {
        dateTime: dateTime,
    ...
    request.execute(function (event) {
      ...
      if (event) {
        ...
        axios.put(
          `${process.env.REACT_APP_SERVER_URL}/agendamentos/add-meet-link`,
          {
            agendamentoId: id,
  ...
  const { dateId, clinica, slotId } = props.location.data;

  const bookSlot = async () => {
    const { data } = await Axios.post(
      `${process.env.REACT_APP_SERVER_URL}/clinicas/book-slot/`,
      {
    ...
    if (data.clinicaEmail) {
      createEvent(data._id, data.date + "T" + data.slotTime, data.clinicaEmail);
    }
  };

  useEffect(() => {
    setFinalBalnce(1.18 * clinica.feesPerSession);
  }, []);

  const makePayment = async (token) => {
    const { data } = await Axios.post(
      `${process.env.REACT_APP_SERVER_URL}/pacientes/payment`,
      {
        token,
        finalBalnce,
      }
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td className="text-center">
                            <p>
                              <strong>{clinica.feesPerSession}</strong>
                            </p>
                            <p>
                              <strong>{0.18 * clinica.feesPerSession}</strong>
                            </p>
                          </td>
                        </tr>
                        <tr>
