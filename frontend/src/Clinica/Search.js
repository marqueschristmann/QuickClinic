...
const Search = () => {
  ...
  const fetchClinica = async () => {
    const { data } = await Axios.get(
      `${process.env.REACT_APP_SERVER_URL}/clinicas/`
    );
    setClinica(data);
    console.log(data);
  };
  ...
  const UpdateDisplay = (text) => {
    setClinica((Clinica) => {
      return Clinica.filter(
        (clinica) => clinica.specialization.toLowerCase() === text.toLowerCase()
      );
    });
    console.log(Clinica);
  };
        ...
        <Col>
          <InputGroup>
            <Input
              value={text}
              type="text"
              placeholder="Procure sua Clinica"
      ...
      >
        <div className="row">
          {Clinica.map((cl) => (
            ...
            <div className="col-sm-6 mb-2" key={cl._id}>
              <div className="card">
                <div className="card-body">
                  <div className="text-info">
                    <h6>
                      Clinica Name:
                      <span className="text-uppercase"> {cl.name}</span>
                    </h6>
                  </div>
                  <div>Specialization : {cl.specialization}</div>
                  <div>Phone Number : {cl.phoneNumber}</div>
                  <div className="row mb-0 pb-0">
                    <div className="col-md-6 ">
                      FeesPerSession: {cl.feesPerSession}
                    </div>
                    <div
                      className=" col align-self-end col-md-2 offset-md-3 inline"
                      style={{ textAlign: "center" }}
                    ><Link to={{ pathname: "/paciente/selectdate", clinica: { clinica: cl } }}>
